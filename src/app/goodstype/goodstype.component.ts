...
@Component({
  ...
  styleUrls: ['./goodstype.component.sass'],
...
export class GoodstypeComponent implements OnInit {
  ...
  fetch(cb) {
    ...
    this.request.getgoodstype().subscribe((response) => {
  ...
  editRow(row, rowIndex, content) {
    ...
    this.request.fetchgoodstypeById(row._id).subscribe((response) => {
      this.editcustmergroup=response[0];
      console.log(response);
        this.GoodsNamenamevalue=this.editcustmergroup.GoodsName;
        this.IdValue=this.editcustmergroup._id;
      this.editForm.setValue({
        GoodsName: this.GoodsNamenamevalue,
      });
  ...
  deleteRecord(row) {
    ...
    this.request.deletegoodstype(row._id).subscribe((response) => {
      console.log(response);
      this.viewdata();
     }, (error) => {
       console.log(error);
  ...
  onAddRowSave(form: FormGroup) {
    ...
    this.request.addgoodstype(form.value).subscribe((res: any) => {
...
viewdata(){
  ...
  this.request.updategoodstype(this.IdValue,edata).subscribe((res : any) => {
    if (res.status == 'success') {
      this.modalService.dismissAll();
      this.viewdata();
