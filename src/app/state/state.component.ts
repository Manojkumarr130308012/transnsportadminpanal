...
export class StateComponent implements OnInit {
 ...
 custmerStateNamevalue:any;
 custmerStateCodevalue:any;
  ...
  fetch(cb) {
    ...
    this.request.getstate().subscribe((response) => {
  ...
  editRow(row, rowIndex, content) {
    ...
    this.request.fetchstateById(row._id).subscribe((response) => {
      this.editcustmergroup=response[0];
      console.log(response);

  
        this.custmerStateNamevalue=this.editcustmergroup.StateName;
        this.custmerStateCodevalue=this.editcustmergroup.StateCode;
      ...
      this.editForm.setValue({
        StateName: this.custmerStateNamevalue,
        StateCode: this.custmerStateCodevalue,
      });
  ...
  deleteRecord(row) {
    ...
    this.request.deletestate(row._id).subscribe((response) => {
      console.log(response);
      this.viewdata();
     }, (error) => {
       console.log(error);
  ...
  onAddRowSave(form: FormGroup) {
    ...
    this.request.addstate(form.value).subscribe((res: any) => {
...
viewdata(){
  ...
  this.request.updatestate(this.IdValue,edata).subscribe((res : any) => {
    if (res.status == 'success') {
      this.modalService.dismissAll();
      this.viewdata();
